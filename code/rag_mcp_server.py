...
def rag_search_raw(query: str, top_k: int = 5):
    ...
    for r in sag_results:
        if r['method'] == 'entity-exact' and r['source'] not in seen:
            seen.add(r['source'])
            merged.append({
    ...
    for r in sag_results:
        if r['method'] == 'entity-hop' and r['source'] not in seen:
            seen.add(r['source'])
            merged.append({
                'source': r['source'], 'text': r.get('text', ''),
...
def rag_answer(query: str) -> str:
    ...
    results = rag_search_raw(query, top_k=5)
    if not results:
...
def main():
    ...
    try:
        ...
        _sys.stderr.write(f"[rag-mcp] 预热失败: {_e}\n")
    _sys.stderr.flush()
