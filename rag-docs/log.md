...
- Sources: 145 FANUC 官方手册（B-XXXXXEN/CM 系列），rag-docs/chunks 批次
- Raw PDFs location: /mnt/f/project/*/
- Vector DB: /home/eric_jia/rag_chromadb (wiki_docs)/edoc_v10_m3

## [2026-04-26] ingest | Initial PDF source catalog
- Source: rag-docs/chunks/*.json metadata
...
## [2026-07-17] migrate | 基线迁移 ~/wiki → /mnt/d/知识库wiki/rag-docs
- Source: /home/eric_jia/wiki (160 files, 12MB)
- Destination: /mnt/d/知识库wiki/rag-docs
- Method: rsync -a (1:1 复制, 保留元数据)
- Path updates: SCHEMA.md + log.md 路径引用统一改为 rag-docs/
- Forced cleanup:
  * raw/articles / raw/transcripts / raw/assets (空) → 各加 README.md 占位
- 待办: 旧 ~/wiki 软退役 (改名 ~/wiki_archived_2026-07-17) — 用户授权时执行

### verify phase
- 目录结构 5/5 ✓ (comparisons/concepts/entities/queries/raw/papers)
- raw/papers: 144 catalog .md + 1 PDF (FANUC_Robot_i_series_brochure_zh_2021)
- entity/concept 总 12 页, frontmatter 全合规 (title/type ✓)
- log.md 7 entries (create×3 + ingest×3 + migrate×1)
- i_series metadata source: 旧 /home/eric_jia/wiki/... → 新 /mnt/d/知识库wiki/rag-docs/raw/papers/... (12 chunks coll.update)
- RAG hybrid_search "上海发那科 Robot i" → top-1 = i_series brochure ✓
- BM25 stale auto-rebuild 触发 (N=207355→200854)
